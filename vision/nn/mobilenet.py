...
def MobileNet(*args, **kwargs):
    ...
    return myMobileNet(*args, **kwargs)
...
def myMobileNet(input_shape=None,
                alpha=1.0,
                depth_multiplier=1,
                dropout=1e-3,
                include_top=True,
...
def _depthwise_conv_block(inputs, pointwise_conv_filters, alpha,
    ...
    if strides == (1, 1) or block_id == 4 or block_id == 12:
        x = inputs
    else:
        x = layers.ZeroPadding2D((1, 1),
                                 name='conv_pad_%d' % block_id)(inputs)
    x = layers.DepthwiseConv2D((3, 3),
                               padding='same' if strides == (1, 1) or block_id == 4 or block_id == 12 else 'valid',
